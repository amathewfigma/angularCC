...
async function createCodeConnectFiles({ linkedNodeIdsToFilepathExports, figmaFileUrl, unconnectedComponentsMap, outDir: outDirArg, projectInfo, cmd, }) {
    for (const [nodeId, filepathExport] of Object.entries(linkedNodeIdsToFilepathExports)) {
        ...
        let propMapping = undefined;
        let reactTypeSignature = undefined;
        if (projectInfo.config.parser === 'react' && filepath && exportName) {
            const result = (0, prop_mapping_1.extractSignatureAndGeneratePropMapping)({
                filepath,
                exportName,
