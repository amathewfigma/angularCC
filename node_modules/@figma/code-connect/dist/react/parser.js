...
exports.parseReactDoc = exports.getDefaultTemplate = exports.replacePropPlaceholders = exports.parseRenderFunction = exports.parseComponentMetadata = exports.extractComponentTypeSignature = exports.findAndResolveImports = void 0;
...
function findCallExpression(node) {
    if (typescript_1.default.isCallExpression(node)) {
        ...
        return typescript_1.default.forEachChild(node, findCallExpression);
    }
}
/**
 ...
 * @returns
 */
function findDescendantArrowFunction(node) {
    if (typescript_1.default.isArrowFunction(node)) {
        return node;
    }
    else {
        return typescript_1.default.forEachChild(node, findDescendantArrowFunction);
    }
}
/**
 ...
 * @returns
 */
function findDescendantFunctionExpression(node) {
    if (typescript_1.default.isFunctionExpression(node)) {
        return node;
    }
    else {
        return typescript_1.default.forEachChild(node, findDescendantFunctionExpression);
    }
}
 ...
 * @param checker
 */
function findFunctionExpression(declaration, checker, sourceFile) {
...
function extractComponentTypeSignature(symbol, checker, sourceFile) {
    ...
    for (const prop of props) {
        ...
        if (typescript_1.default.isInterfaceDeclaration(parent)) {
            const parentInterfaceName = parent.name.getText();
            if (ReactInterfaceNames.includes(parentInterfaceName) ||
        ...
        propsMap[prop.name] =
            prop.flags & typescript_1.default.SymbolFlags.Optional ? `?${propTypeString}` : propTypeString;
    }
    return propsMap;
}
exports.extractComponentTypeSignature = extractComponentTypeSignature;
/**
